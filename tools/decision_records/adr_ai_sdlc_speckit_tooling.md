...
* **Date:** 2026-07-31
* **Issue:** [#3115](https://github.com/eclipse-score/score/issues/3115)
...
flowchart LR
    ...
    subgraph PM["Package Manager (one of three)"]
        APM["APM\napm.yml + apm.lock.yaml\npolicy · SBOM · hashes"]:::primary
        Lola["Lola\n.lola-req\nno lockfile hashes"]:::fallback
        OKIT["OKIT\nflat copy\nno versioning"]:::avoid
    end
    ...
    classDef primary fill:#2e7d32,color:#fff,stroke:#1b5e20
    classDef fallback fill:#f57c00,color:#fff,stroke:#e65100
    classDef avoid   fill:#c62828,color:#fff,stroke:#b71c1c
```

## 2. Options Considered

### 2.1 APM — `microsoft/apm`
...
- Drift detection; active Microsoft-org project (3.3k★, 71 releases).
...
- Smaller community (109★, 8 releases); missing enterprise governance features.

### 2.3 OKIT — `Mumme-IT/okit`
...
- Single contributor (1★, 1 fork, 40 commits, no releases); bus factor of one.
...
| Tool | S-CORE Fit | Recommendation |
|------|-----------|----------------|
| APM  | High      | **USE** — primary packaging tool |
| Lola | Medium    | **WATCH** — fallback only |
| OKIT | Low       | **DO NOT USE** at this maturity |

> **Note:** Status remains **proposed** until a proof-of-concept pilot validates integration with
