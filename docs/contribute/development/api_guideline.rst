...
So, when designing an API, do not only consider the problem you are trying to solve, but also how a user would want to interact with you.
Approaches like test-driven development can support you in taking the perspective of a user.
This in turn means, that the same API is likely not a good fit for multiple programming languages.
