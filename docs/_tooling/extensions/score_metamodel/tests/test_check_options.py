...
class TestCheckOptions:
    ...
    def test_value_violates_pattern_for_required_option(self):
        ...
        verify_log_string(
            logger,
            f'required option `some_required_option` with value `{need["some_required_option"]}` does not follow pattern `{self.NEED_TYPE_INFO[0]["req_opt"][1][1]}`.',
            expect_location=False,
        )
    ...
    def test_value_violates_pattern_for_optional_option(self):
        ...
        verify_log_string(
            logger,
            f'optional option `some_optional_option` with value `{need["some_optional_option"]}` does not follow pattern `{self.NEED_TYPE_INFO_WITH_OPT_OPT[0]["opt_opt"][0][1]}`.',
            expect_location=False,
        )
