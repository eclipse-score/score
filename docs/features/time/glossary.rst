...
.. glossary::
    Clock
      A source of time. A clock produces a progressing sequence of
...
.. glossary::
    ...
    System Clock
      A wall-clock (:term:`UTC`-based) local time base — the OS ``CLOCK_REALTIME``, also reachable through
    ...
    Absolute Time
      ...
      (e.g. :term:`UTC` from GPS). Reading it yields a :term:`Snapshot` carrying an
      :term:`Absolute Time status`. It is exposed to applications through the
      :term:`Absolute Clock`. Its transmission delay — the path delay from the absolute-time
      master to the client — is compensated using a :term:`Delay Tag`.

    Absolute Clock
      The :term:`Clock` interface that provides :term:`Absolute Time`.

    UTC
      Coordinated Universal Time — the primary global civil time standard. It is the reference
      time scale for :term:`Absolute Time` and for the :term:`System Clock` wall-clock time.
