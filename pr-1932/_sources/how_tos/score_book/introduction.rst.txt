...
.. sectionauthor:: Anton Krivoborodov <Anton.Krivoborodov@bmw.de>
...
to the S-Core project or to extend this book.
...
S-Core project is not the first project that tries to solve these kinds of problems. In the past, such projects as Autosar, tried to
...
it tries to go make the next step based on the experience of autosar and comparable standartization projects. The main two goals besides
...
- S-Core project aims to provide not only the specification of the modules, but also the implementation. As we all know,
  ...
  some of the aspects of the implementation, e.g. performance, are hard to be specified. Working on the same implementation in the open-source
  allows to fix the problems directly on the implementation level and to ensure, that they will not happen in the future, as we always
...
- Another important aspect that normally comes up by the integration of the software stack, espeicially in ASIL relevant systems, is its quality
  ...
  test coverage and so on. Therefore is of the most important goals of the S-Core project not only to provide an efficient, state of the art implementation
  of the automotove software platform but also to define an ASIL conform software development process and to ensure, that all implementations of
  ...
  goal is to specify only those parts of software development process, that also can be validated with automated checks.
...
automotive software platform and will start not only reusing but also actively contribution to the S-Core project.
